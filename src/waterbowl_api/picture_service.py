...
class PictureService:
    ...
    async def create_metadata(self, picture_id) -> PictureMetadata:
        db_picture_metadata = PictureMetadata(picture_id=picture_id)
    ...
    async def create_picture(self, picture: UploadFile, timestamp: float) -> Picture:
        ...
        await self.create_metadata(picture_id=db_picture.id)
        await self._db.refresh(db_picture)
        return db_picture
